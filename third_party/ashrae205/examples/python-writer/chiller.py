...
def setup_table(table_name):
    ...
    # populate builder
    code = compile("{0}.{1}.{1}Start(builder)".format(RSID,table_name),"<string>","exec"); exec code
    for e in elements:
        code = compile("{0}.{1}.{1}Add{2}(builder,{3})".format(RSID,table_name,upperfirst(e["name"]),e["value"]),"<string>","exec"); exec code
    code = compile("table_buffer = {0}.{1}.{1}End(builder)".format(RSID,table_name),"<string>","exec"); exec code
    return table_buffer
